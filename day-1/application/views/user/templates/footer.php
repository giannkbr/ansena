<!-- Footer -->
<footer class="bg-white" id="footer">
    <div class="container">
        <div class="row py-5">
            <div class="col-lg py-2">
                <h5 class="card-text yellow-text fw-light">Belanja</h5>
                <p>Menyediakan barang berkualitas.</p>
            </div>
            <div class="col-lg py-2">
                <p class="fw-bold">About Us</p>
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link text-dark p-0 my-1 small" href="#">Service</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-dark p-0 my-1 small" href="#">Portofolio</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-dark p-0 my-1 small" href="#">Contact Us</a>
                    </li>
                </ul>
            </div>
            <div class="col-lg py-2">
                <p class="fw-bold">Contact Us</p>
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <p class="small">Jangan ragu menghubungi kami jika ada pertanyaan :)</p>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link text-dark p-0 my-1 small" href="tel:+6281388912929">Phone: 081388912929</a>

                    </li>
                </ul>
            </div>
            <div class="col text-end mt-auto">
                <!-- <a href="#"><i class="h3 mx-2 text-dark bi bi-facebook"></i></a> -->
                <a target="_blank" href="https://www.instagram.com/Belanja/"><i class="h3 mx-2 text-dark bi bi-instagram"></i></a>
                <!-- <a href="#"><i class="h3 mx-2 text-dark bi bi-tiktok"></i></a> -->
            </div>
        </div>
    </div>
    <div class="container-fluid">
        <div class="row border-top">
            <div class="col text-center">
                <p class="py-3 m-0">Copyright 2022 @ Belanja</p>
            </div>
        </div>
    </div>
</footer>
<!-- Akhir Footer -->